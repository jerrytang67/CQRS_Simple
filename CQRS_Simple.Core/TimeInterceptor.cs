...
namespace CQRS_Simple.Core
{
    ...
    public class CallLogger : IInterceptor
    {
        ...
        public CallLogger(TextWriter output, ILogger<CallLogger> logger)
        {
            _output = output;
            _logger = logger;
        }
        ...
        public void Intercept(IInvocation invocation)
        {
            if (invocation.Method.IsPublic)
            {
                ...
                _logger.LogInformation($"你正在调用方法 \"{invocation.Method.DeclaringType!.Namespace}.{invocation.Method.DeclaringType!.Name}.{invocation.Method.Name}\"  参数是 {0} "
                    ,
                    string.Join(", ", invocation.Arguments.Select(a => (a ?? "").ToString()).ToArray()));
                ...
                _logger.LogInformation($"方法执行完毕，返回结果：运行时间{sw.ElapsedMilliseconds / 100.0:N3}秒");
            }
        }
    }
}